...
(function() {
    ...
    function setup($) {
        ...
        $.blockUI.defaults = { /** message displayed when blocking (use null for no message) */
            message: '<img id="loadingImage" src="/assets/ajax-loader.gif" style="vertical-align:middle" />&nbsp;&nbsp;&nbsp;<span>Processing. Please wait...</span>',
        ...
        function handler(e) { /** allow tab navigation (conditionally) */
            ...
            var opts = e.data; /** allow events within the message content */
...
/**
 * this function shows the default loading message from blockUI
 * @param {dictionary} [params] holds the optional display message and the option parameter to block
 */

function showLoadingMessage(params) {
	
    try {
        if (typeof(params) !== "undefined") {
            $.blockUI.defaults.message = '<img id="loadingImage" src="/assets/ajax-loader.gif" style="vertical-align:middle" />&nbsp;&nbsp;&nbsp;<span>' + (params.message || 'Processing. Please wait.') + '</span>';
        }
        ...
        /** if the loadingImage element still exists, refresh it so the gif doesn't freeze */
        //setTimeout("try{document.getElementById('loadingImage').src = '/assets/ajax-loader.gif';}catch(ex){alert(ex)}", 100);

        if (typeof(params) == "undefined") { /** this will prompt the user to resend the request if the user is left hanging for more than 12 seconds */
            setTimeout("try{alert('Your request has taken too long. Please try again.');hideLoadingMessage();stopPageRequest();}catch(ex){}", 60000);

        } else if (typeof(params.duration) !== "undefined") {
            setTimeout(function() {
                hideLoadingMessage()
            }, params.duration);

        } else if (typeof(params.doNotWait) == "undefined" && typeof(params.callback) == "undefined") {

            /** this will prompt the user to resend the request if the user is left hanging for more than 12 seconds */
            setTimeout("try{alert('Your request has taken too long. Please try again.');hideLoadingMessage();stopPageRequest();}catch(ex){}", 60000);
...
/**
 * stops the page request from being fulfilled
 */

function stopPageRequest() {
    try {
        window.stop();
    } catch (ex) {}
    try { /** for IE */
...
/** this function hides the default loading message from blockUI */
